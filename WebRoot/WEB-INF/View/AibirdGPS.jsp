...
<head>
	...
	<style type="text/css">
		...
		#whole{position:relative;width:100%;height:100%;}
		#l-map{float:left;height:100%;width:80%;}
		#r-result{float:left;width:20%; font-size:14px;line-height:20px;}
	</style>
...
<body>
    ...
    <div id="whole">
	    ...
	    <div class="result" id="r-result">
		    <input type="button" value="无人机所在地" onclick="bdGEO(0)" />
		    <div id="result"></div>
	    </div>
	</div>
</body>
...
<script type="text/javascript">
	...
	var startx1=<%=session.getAttribute("startx")%>;
    var starty1=<%=session.getAttribute("starty")%>;
    var num=<%=session.getAttribute("num")%>;
    var userphone=<%=session.getAttribute("userphone")%>;
	map.centerAndZoom("武汉光谷广场",13);//鼠标测距 
	/* map.centerAndZoom(new BMap.Point(startx1,starty1),13); */
	...
	switch(num){
	    case 47:
			 startx=<%=session.getAttribute("startx"+46)%>;
   			 starty=<%=session.getAttribute("starty"+46)%>;
   			 userphone[46]=<%=session.getAttribute("userphone"+46)%>;
   			 adds[46]=new BMap.Point(startx,starty);
   			 aibirdid[46]=<%=session.getAttribute("aibirdid"+46)%>;
	    case 46:
...
<%-- 	for(var i=0;i<num;i++){
     var startx=<%=session.getAttribute("startx"+0)%>;
     var starty=<%=session.getAttribute("starty"+0)%>;
     adds[i]=new BMap.Point(startx,starty);
   	} --%>
	...
	for(var i = 0; i<adds.length; i++){
		...
		marker.setAnimation(BMAP_ANIMATION_BOUNCE); //跳动的动画  userphone
		var content=aibirdid[i];
		addClickHandler(content,marker);//给每个点添加信息框
	...
	function addClickHandler(content,marker){
	   ...
	   marker.addEventListener("click",function(e){
	      window.location.href="http://10.152.120.130:8080/AibirdMap/aibird/getAibirdGPS?aibirdid="+content;
	   });
	}
	...
	function geocodeSearch(pt){
		...
		myGeo.getLocation(pt, function(rs){
			...
			document.getElementById("result").innerHTML +=index + ". "+"手机号:  "+userphone[index-1]+"    ,"+adds[index-1].lng + "," + adds[index-1].lat + "："  + "商圈(" + rs.business + ")  结构化数据(" + addComp.province + ", " + addComp.city + ", " + addComp.district + ", " + addComp.street + ", " + addComp.streetNumber + ")<br/><br/>";
		});
	}
